...
3. Go to **Applications** to search and enable the following:
     ...
     * **Kserve** : version ``v0.19.0`` or higher with the following ``Values`` configuration 
...
4. Check if ``Kserve`` and ``Kserve LLMInferenceService`` resources are ready either in the GUI by watching for ``Running`` state or in the commandline as follows:
...
3. Login using `admin` user and password that was set in the install workflow
